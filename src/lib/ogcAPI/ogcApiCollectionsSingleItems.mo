...
module {
    ...
    public func getCollectionsSingleMapItems (map: TM.TrackMap, canister: Principal, local: Bool, repr: H.Representation ): Text {
        if (repr == #json) {
            return getCollectionsSingleMapItemsJSON(map, canister, local);
        };
        return getCollectionsSingleMapItemsHTML(map, canister, local);
    };

    private func getCollectionsSingleMapItemsJSON (map: TM.TrackMap, canister: Principal, local: Bool) : Text {
        return map.getGeoJSONLineCollection ();
    };

    private func getCollectionsSingleMapItemsHTML (map: TM.TrackMap, canister: Principal, local: Bool) : Text {
        return htmlSkeletonCollection(map.getGeoJSONLineCollection (), map.getBBox(), "FC", canister, local);       
    };

    // The Items of a single Track
    
    public func getCollectionsSingleTrackItems (track: TR.Track, canister: Principal, local: Bool, repr: H.Representation ): Text {
        if (repr == #json) {
            return getCollectionsSingleTrackItemsJSON(track, canister, local);
        };
        return getCollectionsSingleTrackItemsHTML(track,canister, local);
    };

    private func getCollectionsSingleTrackItemsJSON (track: TR.Track, canister: Principal, local: Bool) : Text {
        track.getGeoJSONPointCollection();
    };

    private func getCollectionsSingleTrackItemsHTML (track: TR.Track, canister: Principal, local: Bool) : Text {
        return htmlSkeletonCollection(track.getGeoJSONPointCollectionChunk(0,100), track.getBBox(), track.getTrackId(), canister, local);
    };

    // Helper for HTML Skeleton
    // Genetates the page with a JSON and the ID
    private func htmlSkeletonCollection (geoJSON: Text, extent: BBox.BBox, collectionID : Text, canister: Principal, local: Bool) : Text {
       // head
        ...
        navContent #= HTML.create_A(collectionID, HTTP.buildUrl("/collections/" # collectionID, canister, local, #html), null, null);
        navContent #= HTML.create_A("Items", HTTP.buildUrl("/collections/" # collectionID # "/items", canister, local, #html), null, ?"current");
        ...
        navContent #= HTML.create_A("JSON", HTTP.buildUrl("/collections/" # collectionID # "/items", canister, local, #json), null, ?"JSON");
