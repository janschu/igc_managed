...
module {
    ...
    public class TrackMap () {
        ...
        public func getTracklist () : [Text] {
            ...
            return Buffer.toArray(tracklist);
        };
        ...
        public func deleteTrackById (trackId: Text) : ? Text {
            switch (tracks.remove(trackId)) {
                case (?track) {
        ...
        public func getBBox () : BBox.BBox {
            var box : BBox.BBox = {minLat=0; minLon=0; maxLat=0; maxLon=0};
            let iterTracks : I.Iter<TR.Track> = tracks.vals();
            I.iterate<TR.Track>(iterTracks, func(track, _index) {
                if (_index==0) {
                    box := track.getBBox();
                } else {
                    box := BBox.extendBBox(box,track.getBBox());
                }
            });
            return box;
        };
